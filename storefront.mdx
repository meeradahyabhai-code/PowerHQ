...
Visitors compare plans, sign up and complete enrollment without leaving you. PowerHQ handles provider
...
The Sandbox environment uses the same structure on `www.dev.powerhq.co`. Production links are identical
...
Put the iframe on a test page and run the flow as a customer would. Confirm:
...
The Storefront is the recommended path for most partners. If the energy experience has to live natively inside a product you already maintain, see the [PowerHQ API](/introduction) instead, or [compare the two side by side](/integrations).
