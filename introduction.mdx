...
<Note>
  Your API key is the only credential — there is no IP allow-listing, so you can call the API from anywhere. Contact your PowerHQ representative for a key, or run real queries with no key at all in the [playground](/playground). Requests without a valid key are rejected with `403 Forbidden`.
</Note>
