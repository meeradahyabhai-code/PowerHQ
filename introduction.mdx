---
title: "PowerHQ API: Retail Electricity Data for Partners"
...
<CardGroup cols={2}>
  ...
  <Card title="Utilities" icon="bolt" href="/queries/utilities">
    Look up the transmission and distribution utility (TDU) that serves a given ZIP code, and use its id to request plans once per utility.
  </Card>
...
<Note>
  Your API key is the only credential. Contact your partner manager for a key, or run real queries with no key at all in the [public sandbox](/playground), which runs on live production plans. Requests without a valid key are rejected with `403 Forbidden`.
</Note>
