...
<ParamField query="annualUsageInkWh" type="Float">
  Annual electricity usage in kWh for the business. Supplying this value ensures that the returned `price` reflects the provider's rate at the customer's actual usage level. Omit only when usage is unknown.
</ParamField>
...
<ResponseField name="supplier" type="Supplier">
  ...
  <Expandable title="supplier fields">
    ...
    <ResponseField name="shortName" type="String">
      Abbreviated or display name of the provider, suitable for UI labels. May be `null`.
    </ResponseField>
...
<ResponseField name="agreementUrl" type="String">
  Direct URL to the provider's contract PDF for this plan. Display or download this document so the customer can review the full terms of service before enrolling.
</ResponseField>
