...
description: "Retrieve the utility or TDU serving a given ZIP code. Use the returned utility ID to fetch residential plans for that specific utility."
---

The `utilities` query returns the electric utility or Transmission and Distribution Utility (TDU) serving a given ZIP code. Run this query first whenever you need to resolve a valid `utilityId` before fetching residential plans — it ensures you're querying plans for the exact utility that serves your customer's address.
