...
<ParamField query="utilityId" type="ID">
  ...
  Plans are the same for every ZIP code a utility serves, so one call per utility covers its whole territory. Prefer `utilityId` over `zipCode` when you cache results, and always use it when a ZIP code is served by more than one utility. See [Call volume and caching](/concepts/call-volume).
</ParamField>
