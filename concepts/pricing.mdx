...
|---|---|
| `advertisedPriceUsdPerKwh` | The static EFL price at exactly 500, 1,000 or 2,000 kWh, set by the supplier. A fixed benchmark, not a reflection of real usage. |
| `allInRateUsdPerKwh` | The per-kWh cost against the customer's real usage pattern, **seasonalized by default**. The most accurate reflection of what they will pay. |
...
| Field | What it represents |
|---|---|
| `advertisedPriceUsdPerKwh` | The static EFL price at that benchmark, set by the supplier. Outside Texas this is the variable supply rate only, excluding any fixed monthly fee. |
