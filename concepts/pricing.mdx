...
Read `ENERGY_CHARGE` when you want to show the supply rate on its own, separately from delivery. Use `advertisedPriceUsdPerKwh` for the headline rate a customer compares on, and `allInRateUsdPerKwh` for what they actually pay once credits and fixed charges are folded in.

<Warning>
  Do not add the fee entries together to reconstruct `allInRateUsdPerKwh` yourself. Bill credits and usage thresholds make the arithmetic usage-dependent, and the `rates` array already carries the computed values at 500, 1000 and 2000 kWh.
</Warning>
