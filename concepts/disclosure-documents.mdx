---
title: "Disclosure Documents: EFL, TOS, YRAC, and More by Market"
...
Note also that the document URLs are the provider's own, not PowerHQ-hosted, so they can move without
...
<Note>
`documents[]` can legitimately be empty on some plans. An empty array is not an error — it simply means the provider has not associated any disclosure documents with that plan in the system. Continue to display the plan normally; just omit the disclosures section of your UI for that plan.
</Note>
