...
Pass your API key in the `x-api-key` HTTP header on every request. The key is a static secret issued by your PowerHQ representative; treat it like a password and never expose it in client-side code or public repositories. Each key has its own request quota and rate limit. See [Rate limits](/reference/errors#rate-limits).
