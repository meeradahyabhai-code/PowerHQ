...
<Tip>
  Want to try the API before you have a key at all? The [sandbox](/playground) runs real queries against live production plans with no key and no signup.
</Tip>

## Environments

Both environments enforce the same authentication requirements, with a separate key for each. Use Sandbox while you build and test. It returns test plans, not live prices. Switch to Production for live customer traffic.
