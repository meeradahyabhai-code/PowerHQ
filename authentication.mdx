...
Pass your API key in the `x-api-key` HTTP header on every request. The key is a static secret issued by your partner manager; treat it like a password and never expose it in client-side code or public repositories.
...
<Tip>
  Want to try the API before you have a key at all? The [public sandbox](/playground) runs real queries against live production plans with no key and no signup.
</Tip>

## Environments

Both environments enforce the same authentication requirements, with a separate key for each. Use the Sandbox environment while you build and test. It returns test plans, not live prices. Switch to Production for live customer traffic.

| Environment | Endpoint | Intended Use |
|---|---|---|
| Production | `https://eapi.prod.powerhq.co/graphql` | Live traffic serving real customers |
| Sandbox | `https://eapi.cert.powerhq.co/graphql` | Development, integration testing, and QA |

<Tip>
  Point your integration at the Sandbox environment during development. It lets you iterate safely without risking any impact to production data or customer-facing systems.
</Tip>

Onboarding asks for the IP addresses your team will be testing from, so we can enable them for the
sandbox checkout flow. Sending them early keeps your first end-to-end test from waiting on us.
...
<Warning>
  A `403 Forbidden` response means the `x-api-key` header is missing, malformed, or not a valid key for that environment. Check that the header is present, that the key has no stray whitespace, and that you are using the right key for the environment you are calling — sandbox and production keys are different.
</Warning>
