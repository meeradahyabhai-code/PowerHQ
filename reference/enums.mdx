---
title: "PowerHQ API: GraphQL Enum Values Reference"
sidebarTitle: "Enums"
description: "Reference for all GraphQL enums in the PowerHQ API, covering rate types, fee categories, document link types, and utility account statuses."
---

The PowerHQ API uses several enum types to represent discrete, well-defined sets of values across plan rates, fee structures, document categories, and account status. Each enum value is a string constant returned exactly as shown — your integration should handle unknown values gracefully in case new values are added in future API versions.
...
| `TOS` | Terms of Service — the full legal contract between the customer and the provider. |
| `YRAC` | Your Rights as a Customer — a Texas-mandated consumer rights document. |
| `ENROLLMENT` | A link to the provider's own enrollment website, distinct from the PowerHQ-hosted enrollment flow. |
...
| `COMM_POLICY` | Communication policy document describing how the provider contacts customers. |
| `PAYMENT_TERMS` | Document specifying the provider's payment terms, due dates, and late-payment policies. |
...
| `SCHEDULE` | Charge is applied according to a provider-defined schedule as specified in the service agreement. |
...
| Value | Description |
|-------|-------------|
| `ENERGY_CHARGE` | The provider's raw energy charge in \$/kWh, before utility delivery charges are added. Always carries `applicability: PER_KWH`. |
