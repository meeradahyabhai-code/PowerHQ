---
title: "PowerHQ API: GraphQL Object Type Reference"
sidebarTitle: "Types"
description: "Complete reference for every GraphQL object type, field, and scalar in the PowerHQ API, including nullability rules and field-level notes."
---

The PowerHQ API uses a fully typed GraphQL schema. Every field in a response maps to a concrete type defined in that schema — object types, scalars, or enums. This page documents each object type, its fields, and whether each field is nullable (no `!`) or non-null (marked `!`). Use this reference alongside the [Enums reference](/reference/enums) for the complete picture.
...
<ResponseField name="title" type="String!" required>
  Human-readable plan name as set by the provider.
</ResponseField>
...
<ResponseField name="supplier" type="Supplier!" required>
  The energy provider offering this plan. See [`Supplier`](#supplier).
</ResponseField>
...
<ResponseField name="supplierScores" type="SupplierScores">
  Aggregated ratings for the provider. Nullable — absent when the provider has no ratings on file. See [`SupplierScores`](#supplierscores).
</ResponseField>
...
<ResponseField name="isPetFriendly" type="Boolean!" required>
  `true` if the provider has a pet-friendly policy or donates to animal welfare causes.
</ResponseField>
...
<ResponseField name="supplier" type="Supplier!" required>
  The provider offering this plan. See [`Supplier`](#supplier).
</ResponseField>
...
<ResponseField name="advertisedPriceUsdPerKwh" type="Float!" required>
  The provider's published EFL price in $/kWh at this fixed usage benchmark, not adjusted for how a customer's usage varies across the year.
</ResponseField>
...
The retail electricity provider offering a plan.
...
<ResponseField name="id" type="ID!" required>
  Unique identifier for the provider in the PowerHQ system.
</ResponseField>

<ResponseField name="name" type="String!" required>
  Full legal or trade name of the provider.
</ResponseField>
...
<ResponseField name="logoUrl" type="String!" required>
  URL to the provider's logo image (PNG or SVG).
</ResponseField>

<ResponseField name="registrationId" type="String">
  Regulatory registration or license number for the provider. Nullable.
</ResponseField>
...
Aggregated ratings for a provider across several dimensions. The entire `supplierScores` object is nullable on `ResidentialPlan` — it is absent when the provider has no ratings on file. Individual category fields within this type may also be independently null.
...
<ResponseField name="powerHqRating" type="Float">
  Overall PowerHQ composite score for the provider. Nullable.
</ResponseField>

<ResponseField name="plansAndRates" type="Float">
  Score reflecting the quality and competitiveness of the provider's plan offerings. Nullable.
</ResponseField>
...
<ResponseField name="renewablePlans" type="Float">
  Score reflecting the availability and quality of the provider's renewable energy options. Nullable.
</ResponseField>
...
<ResponseField name="value" type="String">
  The provider's own description of the benefit, and in practice the field to display. For
