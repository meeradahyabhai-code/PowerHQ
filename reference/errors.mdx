...
| `429 Too Many Requests` | Your API key exceeded its limit of 100,000 requests per month | See [Rate limits](#rate-limits). |

<Warning>
  A `403 Forbidden` means the `x-api-key` header is missing, malformed, or not valid for that environment. Sandbox and production keys are different, so a key that works against the Sandbox environment will 403 against production.
</Warning>
...
Each API key can make 100,000 requests per month. Requests over the limit return `429 Too Many Requests`.
...
- `supplierScores` — the entire object is `null` when the provider has no ratings on file; individual score categories within the object (`powerHqRating`, `plansAndRates`, `customerService`, `renewablePlans`, `pucRating`) may also be independently `null`
