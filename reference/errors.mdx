---
title: "PowerHQ Plan Data API: Error Handling and Validation"
sidebarTitle: "Errors"
...
| `429 Too Many Requests` | You exceeded your API key's request rate limit or monthly quota | Back off and retry with increasing delays. See [Rate limits](#rate-limits). |
...
Each API key has a monthly request quota and a request rate limit. Every request counts once toward the quota, however many plans it returns.

When you exceed either limit, the API returns `429 Too Many Requests` and does not run the query.

- **Back off and retry.** Wait before retrying, and increase the wait after each `429`.
- **If `429` responses continue,** you have likely used your monthly quota. Contact your partner manager to raise it.
- **Cache plans instead of requesting them again.** Plans belong to a utility, not a ZIP code, so one request per utility covers its whole territory. See [Caching and call volume](/queries/residential-plans#caching-and-call-volume).
