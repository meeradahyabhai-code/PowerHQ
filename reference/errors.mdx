...
<Warning>
  A `403 Forbidden` means the `x-api-key` header is missing, malformed, or not valid for that environment. Where the request originated from does not matter — IP allow-listing is not used. Staging and production keys are different, so a key that works against certification will 403 against production.
</Warning>
