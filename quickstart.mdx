...
<Steps>
  ...
  <Step title="Get your credentials">
    You need one thing: **an API key**, a static secret you send on every request in the `x-api-key` HTTP header. There is no IP allow-listing to arrange and no static egress IP to reserve — the key alone authenticates the call, so it works from serverless functions, containers and CI just as well as from a fixed server.
