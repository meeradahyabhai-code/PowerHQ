...
<Info>
  **10 calls per IP per day.** Loading the playground and browsing the schema are free — only real
  queries count. [Request an API key](mailto:support@powerhq.co) to call production directly with no cap.
</Info>
